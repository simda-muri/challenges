...
These are not observations of the sea ice thickness :math:`h_i`, but related to the snow depth :math:`h_s`, sea ice freeboard :math:`h_{if}` (the extension of sea ice above sea level not including the snow depth), and total freeboard :math:`h_f` (the extension of sea ice above sea level including the snow depth), where 
